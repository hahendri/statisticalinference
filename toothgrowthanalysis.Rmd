...
Figure 1 shows that Orange Juice may have increased tooth growth overall.  The 1mg/day doseage 
shows a significant increase, which we will test.
...
##Run t-test
t.test(datoj$len - datvc$len, mu = 0, paired = FALSE, alternative = "two.sided", conf.level = 0.95)
```
...
t.test(datojd3$len - datvcd3$len, mu = 0, paired = FALSE, alternative = "two.sided", conf.level = 0.95)
```
...
```{r}
t.test(datojd1$len - datvcd1$len, mu = 0, paired = FALSE, alternative = "two.sided", conf.level = 0.95)
```

Test of doseage 1 mg/day:

```{r}
t.test(datojd2$len - datvcd2$len, mu = 0, paired = FALSE, alternative = "two.sided", conf.level = 0.95)
```
